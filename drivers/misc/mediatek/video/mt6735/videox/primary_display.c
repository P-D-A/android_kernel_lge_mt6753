...
int primary_display_esd_recovery(void)
{
	...
	DISPCHECK("[POWER]lcm suspend[end]\n");
#else
	if(lge_get_mfts_mode() == 1 && mfts_lpwg != 1)
	{
		disp_lcm_suspend_for_mfts(pgc->plcm);
	}
	else
	{
		disp_lcm_suspend_for_touch(pgc->plcm);
	}
#endif
...
#if defined(TARGET_MT6735_M1V) || defined(TARGET_MT6753_M4)
	{
		LCM_PARAMS *lcm_param = NULL;
		lcm_param = disp_lcm_get_params(pgc->plcm);
		if (lge_get_mfts_mode() == 1 && mfts_lpwg != 1)
		{
			printk("[POWER]lcm resume_mfts[begin]\n");
			disp_lcm_resume_for_mfts(pgc->plcm);
			printk("[POWER]lcm resume_mfts[end]\n");
		}
		else
		{
			printk("[POWER]lcm resume_power[begin]\n");
			disp_lcm_resume_power(pgc->plcm);
			printk("[POWER]lcm resume_power[end]\n");
		}
	}
#endif
	...
	DISPCHECK("[ESD]lcm resume [begin]\n");
	disp_lcm_resume(pgc->plcm);
	DISPCHECK("[ESD]lcm resume [end]\n");
...
int primary_display_suspend(void)
{
	...
	else
	{
		...
		DISPCHECK("[POWER]lcm shutdown[begin]\n");
		disp_lcm_shutdown(pgc->plcm);
		DISPCHECK("[POWER]lcm shutdown[end]\n");
#endif
	}
#endif
...
#if defined(TARGET_MT6735_M1V) || defined(TARGET_MT6753_M4) || defined(TARGET_MT6735_K6)
	{
		LCM_PARAMS *lcm_param = NULL;
		lcm_param = disp_lcm_get_params(pgc->plcm);
		if (lge_get_mfts_mode() == 1 && mfts_lpwg != 1)
...
{
	return ALIGN_TO(DISP_GetScreenWidth(), MTK_FB_ALIGNMENT) * ALIGN_TO(DISP_GetScreenHeight(), MTK_FB_ALIGNMENT) *
