...
unsigned short syncShutter_ov8858_m4;
...
static void OV8858_read_otp_lenc(void)
{
	...
	printk("OV8858_read_otp_lenc Start\n");

	if(pOTP_Data != NULL) {
		printk("[OV8858_read_otp_lenc] OTP LenC Data already read\n");
		return;
	}

	pOTP_Data = kzalloc(sizeof(kal_uint8)*240, GFP_KERNEL);
	if(pOTP_Data == NULL) {
		printk("[OV8858_read_otp_lenc] Memory allocation fail\n");
		return;
	}
	...
	if(flag == 1){
		...
		printk("[OV8858_read_otp_lenc] flag_lenc error : %d\n",flag);
		if(pOTP_Data != NULL){
			kfree(pOTP_Data);
			pOTP_Data = NULL;
		}
	...
	// Read LenC Checksum
	ReadCheckSum = ((read_cmos_sensor(end_addr+1)&0xFF)  |  ((read_cmos_sensor(end_addr+2)&0xFF) << 8));
	printk("[OV8858_read_otp_lenc] checksum = 0x%x, ReadCheckSum = 0x%x\n",checksum,ReadCheckSum);

	if(checksum != ReadCheckSum) {
		printk("[OV8858_read_otp_lenc] checksum error\n");
		if(pOTP_Data != NULL) {
			kfree(pOTP_Data);
			pOTP_Data = NULL;
		}
	...
	printk("OV8858_read_otp_lenc End\n");
	return;
}
static void OV8858_write_otp_lenc(void)
{
	kal_uint16 loop = 0, temp = 0;
	printk("OV8858_write_otp_lenc Start\n");
	...
	printk("OV8858_write_otp_lenc End\n");
	return;
}
static void OV8858_read_otp_awb(void)
{
	...
	for(loop=0; loop < 3; loop++) {
		...
		printk("[OV8858_read_otp_awb] gOTP_AWB_Data[%d]=%d\n",loop,gOTP_AWB_Data[loop]);
	}

	// Read LenC Checksum
	ReadCheckSum = ((read_cmos_sensor(end_addr+1) & 0xFF)  |  ((read_cmos_sensor(end_addr+2)& 0xFF) << 8));
	printk("[OV8858_read_otp_awb] checksum = 0x%x, ReadCheckSum = 0x%x\n",checksum,ReadCheckSum);
...
static void OTP_LENC(void)
{
           ...
           printk("OV8858_OTP_LENC Start\n");

           pOTP_Data = kzalloc(sizeof(kal_uint8)*240, GFP_KERNEL);
           if(pOTP_Data == NULL) {
                     printk("[OV8858_OTP_LENC] Memory allocation fail\n");
                     return;
           }
           ...
           if(flag == 1){
                     ...
                     printk("[OV8858_OTP_LENC] flag error : %d\n",flag);
                     if(pOTP_Data != NULL)
                                kfree(pOTP_Data);
                     return;
           }
           ...
           // Read LenC Data
           checksum = 0;
		   printk("OV8858_OTP_LENC Read Start\n");
           for(loop=0; loop < 240; loop++) {
                     pOTP_Data[loop]= (read_cmos_sensor(start_addr+loop) & 0xFF);
                     checksum += pOTP_Data[loop];
           }
		    printk("OV8858_OTP_LENC Read End\n");

           // Read LenC Checksum
           ReadCheckSum = ((read_cmos_sensor(end_addr+1)&0xFF)  |  ((read_cmos_sensor(end_addr+2)&0xFF) << 8));
           printk("[OV8858_OTP_LENC] checksum = 0x%x, ReadCheckSum = 0x%x\n",checksum,ReadCheckSum);

           if(checksum != ReadCheckSum) {
                     LOG_INF("[OV8858_OTP_LENC] checksum error\n");
                     if(pOTP_Data != NULL)
                                kfree(pOTP_Data);
                     return;
           }
           ...
           // Write LenC Data
		   printk("OV8858_OTP_LENC Write Start\n");
           for(loop=0; loop < 240; loop++) {
                     write_cmos_sensor(0x5800+loop , pOTP_Data[loop]);
           }
		   printk("OV8858_OTP_LENC Write End\n");
...
//	mdelay(10);
	...
	
	{
		...
		if(isOTPLenC_CheckSumPASS == FALSE)
			printk("[OV8858_OTP_Error] please check the ov8858 module\n");
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_SET_ESHUTTER:
            if( (imgsensor_info.ae_shut_delay_frame ==0) && (imgsensor_info.ae_sensor_gain_delay_frame==0) )
            {
                syncShutter_ov8858_m4= (unsigned short)*feature_data;
                LOG_INF("SENSOR_FEATURE_SET_ESHUTTER syncShutter = %d\n",syncShutter_ov8858_m4);
            }
            else
            {
        ...
        case SENSOR_FEATURE_SET_GAIN:
			if( (imgsensor_info.ae_shut_delay_frame ==0) && (imgsensor_info.ae_sensor_gain_delay_frame==0) )
			{
			    set_shutter(syncShutter_ov8858_m4);
			}
