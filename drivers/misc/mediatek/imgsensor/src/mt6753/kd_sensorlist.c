...
#define GPIO_FDD_BAND_SUPPORT_DETECT_2ND_PIN         (GPIO100 | 0x80000000)
#define GPIO_FDD_BAND_SUPPORT_DETECT_2ND_PIN_M_GPIO  GPIO_MODE_00
#define GPIO_FDD_BAND_SUPPORT_DETECT_3RD_PIN         (GPIO101 | 0x80000000)
#define GPIO_FDD_BAND_SUPPORT_DETECT_3RD_PIN_M_GPIO  GPIO_MODE_00
#define GPIO_FDD_BAND_SUPPORT_DETECT_4TH_PIN         (GPIO102 | 0x80000000)
...
// [LGE_UPDATE_S][kipo.yun@lge.com][2015-12-22] 
#if defined(TARGET_MT6753_M4)
int module_flag = -1; // 1:5M Module, 0:8M Module
#endif
// [LGE_UPDATE_E][kipo.yun@lge.com][2015-12-22]
...
static struct i2c_board_info i2c_devs2 __initdata = {I2C_BOARD_INFO(CAMERA_HW_DRVNAME2, 0xfe>>1)};
 ...
 #ifdef TARGET_MT6753_K7
    /*PMIC for sub/main2 camera (borad>=rev.a) */
	struct regulator *regVRF18_1 = NULL; // DVDD_1.2
	struct regulator *regVTCXO_1 = NULL; // AVDD_2.8
	struct regulator *regVGP1 = NULL;    // IODVDD_1.8
 #endif
#endif
...
extern int mtkcam_flash_gpio_init(struct platform_device *pdev); //flash gpio control
...
static DEFINE_MUTEX(kdCam_i2c_Mutex);// [LGE_UPDATE] [kyunghun.oh@lge.com] [2015-11-08] Mod_i2c_tranfer_mode_"start_and_stop"_to_"repeated_start"
...
#if defined (TARGET_MT6753_M4)
#define cam_mutex_lock     mutex_lock
#define cam_mutex_unlock   mutex_unlock
#else
#define cam_mutex_lock(a)
#define cam_mutex_unlock(a)
#endif
...
int iMultiReadReg(u16 a_u2Addr , u8 *a_puBuff , u16 i2cId, u8 number)
{
    ...
    cam_mutex_lock(&kdCam_i2c_Mutex);
	if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
		...
		if (i4RetValue != 2) {
            ...
            cam_mutex_unlock(&kdCam_i2c_Mutex);
			return -1;
		}
		/*  */
...
// [LGE_UPDATE_S][kipo.yun@lge.com][2015-12-22]
#if defined(TARGET_MT6753_M4)
bool is_front_5M_module(void)
{
	int s1=0,s2=0,s3=0,s4=0,model_sel=0;

	mt_set_gpio_mode(GPIO_FDD_BAND_SUPPORT_DETECT_1ST_PIN,GPIO_FDD_BAND_SUPPORT_DETECT_1ST_PIN_M_GPIO);
	mt_set_gpio_dir(GPIO_FDD_BAND_SUPPORT_DETECT_1ST_PIN,GPIO_DIR_IN);
	s1 = mt_get_gpio_in(GPIO_FDD_BAND_SUPPORT_DETECT_1ST_PIN);

	mt_set_gpio_mode(GPIO_FDD_BAND_SUPPORT_DETECT_2ND_PIN,GPIO_FDD_BAND_SUPPORT_DETECT_2ND_PIN_M_GPIO);
	mt_set_gpio_dir(GPIO_FDD_BAND_SUPPORT_DETECT_2ND_PIN,GPIO_DIR_IN);
	s2 = mt_get_gpio_in(GPIO_FDD_BAND_SUPPORT_DETECT_2ND_PIN);

	mt_set_gpio_mode(GPIO_FDD_BAND_SUPPORT_DETECT_3RD_PIN,GPIO_FDD_BAND_SUPPORT_DETECT_3RD_PIN_M_GPIO);
	mt_set_gpio_dir(GPIO_FDD_BAND_SUPPORT_DETECT_3RD_PIN,GPIO_DIR_IN);
	s3 = mt_get_gpio_in(GPIO_FDD_BAND_SUPPORT_DETECT_3RD_PIN);


	mt_set_gpio_mode(GPIO_FDD_BAND_SUPPORT_DETECT_4TH_PIN,GPIO_FDD_BAND_SUPPORT_DETECT_4TH_PIN_M_GPIO);
	mt_set_gpio_dir(GPIO_FDD_BAND_SUPPORT_DETECT_4TH_PIN,GPIO_DIR_IN);	
	s4 = mt_get_gpio_in(GPIO_FDD_BAND_SUPPORT_DETECT_4TH_PIN);
	
	printk("POPO s1=%d  s2=%d  s3=%d  s4=%d\n",s1,s2,s3,s4);

	model_sel = (s4<<3) | (s3<<2) | (s2<<1) | s1;

	printk("POPO model_sel=%d\n",model_sel);

	if((model_sel == 0)||(model_sel == 2)||(model_sel == 6)||(model_sel == 8)||(model_sel == 10)||(model_sel == 14))
	  return 1; //5M Moudle 
	else
	  return 0; //8M Module
}
...
// [LGE_UPDATE_S] [kipo.yun@lge.com] [2014-10-25] AT command added for main/sub camera to prevent mixed items
                    printk(" Sensor found ID Start = 0x%x\n", sensorID);
                    switch (sensorID) {
                        case 0xD850:
                            g_mainSensor = 13850;
	...
	if (1) {
		...
		if (node) {
			...
			if (name == NULL) {
			    ...
			    if (regVTCXO_1 == NULL) {
				    regVTCXO_1 = regulator_get(sensor_device, "vtcxo_1");
			    }
			    if (regVGP1 == NULL) {
				    regVGP1 = regulator_get(sensor_device, "vgp1");
			    }
#endif
			} else{
...
#if defined(TARGET_MT6753_M4)	
    if(module_flag == -1)
       module_flag = is_front_5M_module();
#endif
// [LGE_UPDATE_E][kipo.yun@lge.com][2015-12-22]		
	...
	/* set I2C clock rate */
	g_pstI2Cclient->timing = 400;/* 100k */   //[LGE_UPDATE][kipo.yun@lge.com][2015-11-28] Performance Tuning
...
// [LGE_UPDATE_E][kipo.yun@lge.com][2015-12-22]	
	...
	/* set I2C clock rate */
	g_pstI2Cclient2->timing = 400;/* 100k */ //[LGE_UPDATE][kipo.yun@lge.com][2015-11-28] Performance Tuning
