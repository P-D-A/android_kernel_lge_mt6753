...
static int toMacroSteps(int max_step, int interval, int CurrentPos, int TargetPos)
{
	...
	LOG_INF("[DW9714AF] toMacroSteps from=%d, to=%d \n", CurrentPos, TargetPos);
	...
	while( max_step >= THRESHOLD )
	{
		while( abs(CurrentPos - TargetPos) > max_step )
		{
			...
			LOG_INF("tmpTargetPos=%d\n", tmpTargetPos);
			if(s4AF_WriteReg(tmpTargetPos) == 0)
			{
				CurrentPos = tmpTargetPos;
				LOG_INF("CurrentPos=%d\n", CurrentPos);
			}
			else
			{
				LOG_INF("[DW9714AF] set I2C failed when moving the motor \n");
				g_i4MotorStatus = -1;
				return 0;
			}
			udelay(300);
...
{
	unsigned short tmpTargetPos;
	LOG_INF("[DW9714AF] toInfSteps from=%d, to=%d \n", CurrentPos, TargetPos);
	if(TargetPos <= CurrentPos){
		return 0;
	}
	while( max_step >= THRESHOLD )
...
//[LGE_UPDATE_S][kipo.yun@lge.com][2016-01-09] Stability ISSUE
//			else if( (g_i4Dir == 1) && (g_u4CurrPosition!=g_u4TargetPosition) && (g_u4TargetPosition<700)){
//				toInfSteps(100, 10, g_u4CurrPosition, (g_u4TargetPosition));
//			}
//[LGE_UPDATE_E][kipo.yun@lge.com][2016-01-09] Stability ISSUE
			else
			{
