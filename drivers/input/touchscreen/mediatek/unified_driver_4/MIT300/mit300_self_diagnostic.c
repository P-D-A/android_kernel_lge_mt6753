...
#define CM_DELTA_MAX 9999
#define CM_DELTA_MIN 40
#define INTENSITY_MAX 200
#define INTENSITY_MIN -100
#define JITTER_MAX 50
#define JITTER_MIN 0
#define OPEN_SHORT_MAX 4050
#define OPEN_SHORT_MIN 800
#define MUX_SHORT_MAX 4050
#define MUX_SHORT_MIN 800
#define LPWG_RAW_DATA_MAX 3100
#define LPWG_RAW_DATA_MIN 750
#define LPWG_JITTER_MAX 50
#define LPWG_JITTER_MIN 0
#endif

#if defined(TOUCH_MODEL_M1V)
#define RAW_DATA_MAX 3200
#define RAW_DATA_MIN 700
#define RAW_DATA_MARGIN 0
#define CM_DELTA_MAX 290
#define CM_DELTA_MIN 30
#define INTENSITY_MAX 200
#define INTENSITY_MIN -100
#define JITTER_MAX 45
#define JITTER_MIN 0
#define OPEN_SHORT_MAX 4000
#define OPEN_SHORT_MIN 700
#define MUX_SHORT_MAX 4000
#define MUX_SHORT_MIN 700
#define LPWG_RAW_DATA_MAX 3000
#define LPWG_RAW_DATA_MIN 800
#define LPWG_JITTER_MAX 45
#define LPWG_JITTER_MIN 0
#endif
/****************************************************************************
 * Macros
...
int max_data = 0;
int min_data = 0;
int jitter_average_max = 0;
s16 mit_data[MAX_ROW][MAX_COL];
...
static int  MIT300_PrintData(struct i2c_client *client, char *buf, int* result, int type)
{
	...
	case INTENSITY_SHOW:
        ...
        min_data = mit_data[0][0];
		max_data = mit_data[0][0];
		limit_upper = INTENSITY_MAX;
		limit_lower = INTENSITY_MIN;
	...
	for (row = 0 ; row < MAX_ROW ; row++) {
		...
		for (col = MAX_COL-1 ; col >= 0 ; col--) {
            ...
            if(type == JITTER_SHOW){ //average jitter
                if(col >=7){
                    average_left += mit_data[row][col];
                }else{
                    average_right += mit_data[row][col];
                }
                
                if(col==7){
                    average_temp = average_left/8;
                    if(average_temp>jitter_average_max)
                        jitter_average_max = average_temp;
                    average_left = 0;
                }
                
                if(col==0){
                    average_temp = average_right/7;
                    if(average_temp>jitter_average_max)
                        jitter_average_max = average_temp;
                    average_right = 0;
                }
...
int MIT300_DoTest(struct i2c_client *client, int type){
	...
	if (reg_type == TESTREG) { //Read Test Register (Change test mode)
        ...
        if(type==LPWG_JITTER_SHOW) mdelay(1000);
		/* Wait ready status */
		wait_cnt = 40;
		while(wait_cnt--){
			if(MIT300_GetReadyStatus(client) == MIP_CTRL_STATUS_READY){
