...
#ifdef CONFIG_LGE_PM_BATTERY_PROFILE_FROM_DT
...
int batt_meter_read_profile(struct device_node *np)
{
	struct device_node *np_profile = np;
	const char *batt_id, *node_name;
	unsigned int val;
	int cal_model_size = 0;
	int ret = 0;
	u32 prop_array[MODEL_SIZE];

	batt_id = (const char*)lge_get_battery_id_str();

	ret = of_property_read_string(np, batt_id, &node_name);
	if (!ret) {
		np_profile = of_find_node_by_name(np, node_name);
		bm_err("batt_id: %s, node_name: %s\n", batt_id, node_name);
	} else {
		np_profile = np;
		bm_err("Get batt_id failed\n");
	}

	if (of_property_read_u32(np_profile, "car_tune_value", &val) == 0) {
		batt_meter_cust_data.car_tune_value = (int)val;
		bm_debug("Get car_tune_value: %d\n",
			batt_meter_cust_data.car_tune_value);
	} else {
	...
	if (of_property_read_u32(np_profile, "q_max_pos_50", &val) == 0) {
		batt_meter_cust_data.q_max_pos_50 = (int)val;
		bm_debug("Get q_max_pos_50: %d\n",
			batt_meter_cust_data.q_max_pos_50);
	} else {
	...
	if (of_property_read_u32(np_profile, "q_max_pos_25", &val) == 0) {
		batt_meter_cust_data.q_max_pos_25 = (int)val;
		bm_debug("Get q_max_pos_25: %d\n",
			batt_meter_cust_data.q_max_pos_25);
	} else {
	...
	if (of_property_read_u32(np_profile, "q_max_pos_0", &val) == 0) {
		batt_meter_cust_data.q_max_pos_0 = (int)val;
		bm_debug("Get q_max_pos_0: %d\n",
			batt_meter_cust_data.q_max_pos_0);
	} else {
	...
	if (of_property_read_u32(np_profile, "q_max_neg_10", &val) == 0) {
		batt_meter_cust_data.q_max_neg_10 = (int)val;
		bm_debug("Get q_max_neg_10: %d\n",
			batt_meter_cust_data.q_max_neg_10);
	} else {
	...
	if (of_property_read_u32(np_profile, "q_max_pos_50_h_current", &val) == 0) {
		batt_meter_cust_data.q_max_pos_50_h_current = (int)val;
		bm_debug("Get q_max_pos_50_h_current: %d\n",
			batt_meter_cust_data.q_max_pos_50_h_current);
	} else {
	...
	if (of_property_read_u32(np_profile, "q_max_pos_25_h_current", &val) == 0) {
		batt_meter_cust_data.q_max_pos_25_h_current = (int)val;
		bm_debug("Get q_max_pos_25_h_current: %d\n",
			batt_meter_cust_data.q_max_pos_25_h_current);
	} else {
	...
	if (of_property_read_u32(np_profile, "q_max_pos_0_h_current", &val) == 0) {
		batt_meter_cust_data.q_max_pos_0_h_current = (int)val;
		bm_debug("Get q_max_pos_0_h_current: %d\n",
			batt_meter_cust_data.q_max_pos_0_h_current);
	} else {
	...
	if (of_property_read_u32(np_profile, "q_max_neg_10_h_current", &val) == 0) {
		batt_meter_cust_data.q_max_neg_10_h_current = (int)val;
		bm_debug("Get q_max_neg_10_h_current: %d\n",
			batt_meter_cust_data.q_max_neg_10_h_current);
	} else {
	...
	if (of_property_read_u32(np_profile, "battery_profile_t0_num", &val) == 0) {
		profile_num = ((int)val) * 2;
		cal_model_size = (MODEL_SIZE - profile_num) * 4;
		bm_debug("Get profile_num: %d, cal_model_size : %d\n", profile_num,
				cal_model_size);
	} else {
		bm_err("Get battery_profile_t0_num failed\n");
	}

	if (of_property_read_u32_array(np_profile, "battery_profile_t0",
				prop_array, profile_num) == 0) {
		batt_meter_table_cust_data.battery_profile_t0_size =
			(sizeof(prop_array) - cal_model_size) / sizeof(BATTERY_PROFILE_STRUCT);

		memcpy(&batt_meter_table_cust_data.battery_profile_t0,
				&prop_array, (sizeof(prop_array) - cal_model_size));
	} else {
		bm_err("Get battery_profile_t0 failed\n");
	}

	if (of_property_read_u32_array(np_profile, "battery_profile_t1",
				prop_array, profile_num) == 0) {
	...
	if (of_property_read_u32_array(np_profile, "battery_profile_t2",
				prop_array, profile_num) == 0) {
	...
	if (of_property_read_u32_array(np_profile, "battery_profile_t3",
				prop_array, profile_num) == 0) {
	...
	if (of_property_read_u32_array(np_profile, "battery_profile_temperature",
				prop_array, profile_num) == 0) {
		batt_meter_table_cust_data.battery_profile_temperature_size =
			(sizeof(prop_array) - cal_model_size) / sizeof(BATTERY_PROFILE_STRUCT);

		memcpy(&batt_meter_table_cust_data.battery_profile_temperature,
				&prop_array, (sizeof(prop_array) - cal_model_size));
	} else {
		bm_err("Get battery_profile_temperature failed\n");
	}

	if (of_property_read_u32_array(np_profile, "r_profile_t0",
				prop_array, profile_num) == 0) {
		batt_meter_table_cust_data.r_profile_t0_size =
			(sizeof(prop_array) - cal_model_size) / sizeof(BATTERY_PROFILE_STRUCT);
	...
	if (of_property_read_u32_array(np_profile, "r_profile_t1",
				prop_array, profile_num) == 0) {
		batt_meter_table_cust_data.r_profile_t1_size =
			(sizeof(prop_array) - cal_model_size) / sizeof(BATTERY_PROFILE_STRUCT);
	...
	if (of_property_read_u32_array(np_profile, "r_profile_t2",
				prop_array, profile_num) == 0) {
		batt_meter_table_cust_data.r_profile_t2_size =
			(sizeof(prop_array) - cal_model_size) / sizeof(BATTERY_PROFILE_STRUCT);
	...
	if (of_property_read_u32_array(np_profile, "r_profile_t3",
				prop_array, profile_num) == 0) {
		batt_meter_table_cust_data.r_profile_t3_size =
			(sizeof(prop_array) - cal_model_size) / sizeof(BATTERY_PROFILE_STRUCT);
	...
	if (of_property_read_u32_array(np_profile, "r_profile_temperature",
				prop_array, profile_num) == 0) {
		batt_meter_table_cust_data.r_profile_temperature_size =
			(sizeof(prop_array) - cal_model_size) / sizeof(BATTERY_PROFILE_STRUCT);

		memcpy(&batt_meter_table_cust_data.r_profile_temperature,
				&prop_array, (sizeof(prop_array) - cal_model_size));
	} else {
		bm_err("Get r_profile_temperature failed\n");
	}

	return ret;
}

int __batt_meter_init_battery_profile_from_dt(struct platform_device *dev)
{
	struct device_node *np = dev->dev.of_node;
	unsigned int val;

	bm_err("__batt_meter_init_battery_profile_from_dt\n");
...
/*
	...
	if (of_property_read_u32(np, "is_battery_remove_by_pmic", &val)) {
		...
		bm_err("Get is_battery_remove_by_pmic failed\n");
	}
*/

	if (batt_meter_read_profile(np))
		bm_err("Get mediatek default battery profile data!\n");
	else
		bm_err("Get custom battery profile data!\n");
...
#ifdef CONFIG_LGE_PM_BATTERY_PROFILE_FROM_DT
	return __batt_meter_init_battery_profile_from_dt(dev);
#else
	/* #ifdef CONFIG_OF */
...
#ifdef CONFIG_LGE_PM_BATTERY_OCV_COMPENSATE
signed int fgauge_read_d_by_v(signed int volt_bat)
{
	...
	profile_p = &batt_meter_table_cust_data.battery_profile_t2[0];
	if (profile_p == NULL) {
...
#define DIFF_RTC_HWOCV_H 21
#define DIFF_RTC_HWOCV_M 4
#define DIFF_RTC_HWOCV_L 2

#define DIFF_RTC_SWOCV_H 31
#define DIFF_RTC_SWOCV_M 16
#define DIFF_RTC_SWOCV_L 8

#define COMPENSATE_VAL_H 2
#define COMPENSATE_VAL_M 1
signed int lge_ocv_compensate(int rtc_soc, int voltage) {
	signed int hwocv = 0, swocv = 0;
	signed int compensate_rtc_soc = rtc_soc;
	int diff_rtc_hwocv = 0;
	int diff_rtc_swocv = 0;

	if (get_boot_reason() == BR_POWER_KEY) {
		hwocv = 100 - fgauge_read_d_by_v(voltage);
		diff_rtc_hwocv = abs(rtc_soc- hwocv);
		bm_err("hwocv_voltage : %d, hwocv: %d, original_rtc: %d, diff_rtc_hwocv: %d\n",
				voltage, hwocv, rtc_soc, diff_rtc_hwocv);
		if (diff_rtc_hwocv >= DIFF_RTC_HWOCV_M && diff_rtc_hwocv < DIFF_RTC_HWOCV_H ) {
			if (hwocv > rtc_soc)
				compensate_rtc_soc = rtc_soc;
			else
				compensate_rtc_soc = rtc_soc - COMPENSATE_VAL_H;
		} else if (diff_rtc_hwocv >= DIFF_RTC_HWOCV_L && diff_rtc_hwocv < DIFF_RTC_HWOCV_M) {
			if (hwocv > rtc_soc)
				compensate_rtc_soc = rtc_soc;
			else
				compensate_rtc_soc = rtc_soc - COMPENSATE_VAL_M;
		} else if (diff_rtc_hwocv < DIFF_RTC_HWOCV_L) {
			compensate_rtc_soc = rtc_soc;
		}
	} else if (bat_is_charger_exist() || get_boot_reason() == BR_USB) {
		swocv = 100 - fgauge_read_d_by_v(gFG_voltage);
		diff_rtc_swocv = abs(rtc_soc- swocv);
		bm_err("swocv_voltage : %d, swocv: %d, original_rtc: %d, diff_rtc_swocv: %d\n",
				gFG_voltage, swocv, rtc_soc, diff_rtc_swocv);
		if (diff_rtc_swocv >= DIFF_RTC_SWOCV_M && diff_rtc_swocv < DIFF_RTC_SWOCV_H) {
			if (swocv > rtc_soc)
				compensate_rtc_soc = rtc_soc + COMPENSATE_VAL_H;
			else
				compensate_rtc_soc = rtc_soc;
		} else if (diff_rtc_swocv >= DIFF_RTC_SWOCV_L && diff_rtc_swocv < DIFF_RTC_SWOCV_M) {
			if (swocv > rtc_soc)
				compensate_rtc_soc = rtc_soc + COMPENSATE_VAL_M;
			else
				compensate_rtc_soc = rtc_soc;
		} else if (diff_rtc_hwocv < DIFF_RTC_SWOCV_L) {
			compensate_rtc_soc = rtc_soc;
		}
	}
	bm_err("compensate_rtc : %d\n", compensate_rtc_soc);

	return compensate_rtc_soc;
}
#endif
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
	/* charger will be enabled in battery_common_fg_20.c */
#else /* MTK Original */
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
			/* use SW_OCV when boot with USB */
			if (!g_battery_soc_ready && (get_boot_reason() == BR_USB
						|| bat_is_charger_exist()))
				voltage = gFG_voltage;
#endif
	...
	case FG_DAEMON_CMD_GET_BATTERY_PLUG_STATUS:
		{
			...
			battery_meter_ctrl(BATTERY_METER_CMD_GET_BATTERY_PLUG_STATUS,
					   &plugout_status);
#ifdef CONFIG_LGE_PM
			/* prevent miss unflaged plugout_status */
			if (get_boot_reason() == BR_POWER_KEY || get_boot_reason() == BR_USB)
				plugout_status = 1;
#endif
...
#ifdef CONFIG_LGE_PM_BATTERY_OCV_COMPENSATE
			battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_OCV, &voltage);
			if (!g_battery_soc_ready && rtc_fg_soc != 0) {
				temperature = force_get_tbat(KAL_TRUE);
				bm_err("[fg_res] temperture = %d\n", temperature);
				if (temperature > 9 && temperature < 46)
					rtc_fg_soc = lge_ocv_compensate(rtc_fg_soc, voltage);
			}
#endif
