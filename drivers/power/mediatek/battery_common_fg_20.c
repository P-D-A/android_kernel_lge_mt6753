...
static struct charging_info lcs_info;
static struct charging_rsp lcs_rsp;
#endif
...
extern int LGBM_AtCmd_create_files(struct platform_device*);
#endif
...
#ifdef CONFIG_LGE_PM
static struct wake_lock charger_event_lock;
#endif
...
static DECLARE_WAIT_QUEUE_HEAD(bat_update_wq);
	...
	/* don't need to check charger hv */
#else
...
#ifdef CONFIG_LGE_PM_PSEUDO_BATTERY
		if (BMT_status.pseudo_batt_enabled)
			val->intval = PSEUDO_BATTERY_CAPACITY;
#endif
		break;
...
#ifdef CONFIG_LGE_PM_PSEUDO_BATTERY
		if (BMT_status.pseudo_batt_enabled)
			val->intval = PSEUDO_BATTERY_TEMP * 10;
#endif
		break;
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PSEUDO_BATT:
		if (val->intval)
			BMT_status.pseudo_batt_enabled = 1;
		else
			BMT_status.pseudo_batt_enabled = 0;
	...
	case POWER_SUPPLY_PROP_USB_CURRENT_MAX:
		if (val->intval)
			BMT_status.usb_current_max_enabled = 1;
		else
			BMT_status.usb_current_max_enabled = 0;
		break;
#endif
	default:
		rc = -EINVAL;
...
static ssize_t store_pseudo_batt(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (buf[0] == '0')
		BMT_status.pseudo_batt_enabled = KAL_FALSE;
	else
		BMT_status.pseudo_batt_enabled = KAL_TRUE;

	if (prev_state != BMT_status.pseudo_batt_enabled)
		wake_up_bat();
...
#ifdef CONFIG_LGE_PM_USB_CURRENT_MAX
static ssize_t show_usb_current_max(struct device *dev,struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", BMT_status.usb_current_max_enabled);
}

static ssize_t store_usb_current_max(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	if (!buf)
		return 0;
	...
	if (buf[0] == '0')
		BMT_status.usb_current_max_enabled = KAL_FALSE;
	else
		BMT_status.usb_current_max_enabled = KAL_TRUE;

	return size;
}
static DEVICE_ATTR(usb_current_max, 0664, show_usb_current_max, store_usb_current_max);
#endif
...
static void battery_update(struct battery_data *bat_data)
{
    ...
    if (BMT_status.charger_exist == KAL_TRUE) {
        if (lcs_rsp.btm_state == BTM_HEALTH_OVERHEAT)
            bat_data->BAT_HEALTH = POWER_SUPPLY_HEALTH_OVERHEAT;
        else if (lcs_rsp.btm_state == BTM_HEALTH_COLD)
...
#ifdef CONFIG_LGE_PM_CHARGING_SCENARIO
	} else if ((BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state == CHR_ERROR)) {
		if (lcs_rsp.pseudo_chg_ui)
			bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_CHARGING;
		else
...
#else
#ifdef CONFIG_LGE_PM
	/* read current voltage when battery thread is suspended */
	ret_val = battery_meter_get_battery_voltage(battery_suspended);
#else	/* MTK Original */
...
void mt_battery_GetBatteryData(void)
{
	...
	/* use dummy battery temperature when battery not exist */
	if (BMT_status.bat_exist == KAL_FALSE)
		BMT_status.temperature = 20;
#endif
...
#elif defined(CONFIG_LGE_PM_CHARGING_SCENARIO)
	/* update information for lge_charging_scenario */
	lcs_info.batt_volt = BMT_status.bat_vol * 1000;	/* convert mV -> uV */
	lcs_info.batt_temp = BMT_status.temperature;
	lcs_info.is_charger = BMT_status.charger_exist;

#ifdef CONFIG_LGE_PM_PSEUDO_BATTERY
	/* do not run lge_charging_scenario when pseudo battery mode */
	if (BMT_status.pseudo_batt_enabled) {
		if (BMT_status.bat_charging_state == CHR_HOLD || BMT_status.bat_charging_state == CHR_ERROR)
			BMT_status.bat_charging_state = CHR_CC;
		if (lcs_rsp.btm_state != BTM_HEALTH_GOOD)
			lcs_rsp.btm_state = BTM_HEALTH_GOOD;

		battery_log(BAT_LOG_CRTI, "[BATTERY] Pseudo Battery Mode. bypass LCS.\n");
		return status;
	}
#endif
#ifdef CONFIG_LGE_PM_BATTERY_PRESENT
	/* use dummy temperature when battery is not exist */
	if (BMT_status.bat_exist == KAL_FALSE) {
		lcs_info.batt_temp = 20;
		battery_log(BAT_LOG_CRTI, "[BATTERY] battery not exist. use dummy temperature.\n");
	}
#endif

	/* run lge_charging_scenario */
	lge_monitor_batt_temp(lcs_info, &lcs_rsp);

	/* change charging state */
	if (lcs_rsp.change_lvl != STS_CHE_NONE) {
		switch (lcs_rsp.state) {
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] Charging State changed by battery temperature.\n");
	}
	/* clear charger chanaged flag */
	lcs_info.is_charger_changed = 0;
#else
...
static PMU_STATUS mt_battery_CheckChargerVoltage(void)
{
	PMU_STATUS status = PMU_STATUS_OK;
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
	/* Charging IC will check charger voltage */
#else
#if defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
...
#ifdef CONFIG_LGE_PM_USB_CURRENT_MAX
		/* do not check charging overtime */
		if (BMT_status.usb_current_max_enabled)
...
void do_chrdet_int_task(void)
{
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if ((DISO_data.diso_state.cur_vusb_state == DISO_ONLINE) ||
...
#ifdef CONFIG_LGE_PM_EXTERNAL_CHARGER_SUPPORT
			/* to enable charger correctly, charger must be disabled when unplug the cable */
			battery_charging_control(CHARGING_CMD_ENABLE, &BMT_status.charger_exist);
#endif
			...
			wake_unlock(&battery_suspend_lock);
#ifdef CONFIG_LGE_PM
			/* to make sure that unplug event is processed */
			wake_lock_timeout(&charger_event_lock, msecs_to_jiffies(2000));
#endif
...
int bat_routine_thread(void *x)
{
	...
	while (1) {
		...
		if (BMT_status.charger_type != 0 || BMT_status.UI_SOC2 < 15) {
			ktime = ktime_set(BAT_TASK_PERIOD_LOW_BAT, 0);	/* 10s, 10* 1000 ms */
			pr_err("BAT_TASK_PERIOD_LOW_BAT\n");
		} else {
			ktime = ktime_set(BAT_TASK_PERIOD, 0);	/* 30s, 30* 1000 ms */
			pr_err("BAT_TASK_PERIOD\n");
		}
#else
...
void check_battery_exist(void)
{
	...
	if (baton_count >= 3) {
		...
		if (!BMT_status.bat_exist) {
			...
			pmic_enable_interrupt(35, 1, "BATTERY");
		}
	}
#else
	if (baton_count >= 3) {
...
#ifdef CONFIG_LGE_PM
	wake_lock_init(&charger_event_lock, WAKE_LOCK_SUSPEND, "charger_event");
#endif
#if defined(CONFIG_MTK_PUMP_EXPRESS_SUPPORT) || defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)
...
#endif
#ifdef CONFIG_LGE_PM_USB_CURRENT_MAX
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_usb_current_max);
#endif
#ifdef CONFIG_LGE_PM_AT_CMD_SUPPORT
		ret_device_file = LGBM_AtCmd_create_files(dev);
#endif
	}
