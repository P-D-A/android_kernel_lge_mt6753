...
static int simple_sd_ioctl_get_partition_size(struct msdc_ioctl* msdc_ctl)
{
	...
	case 8:
		...
		break;
	default:
		pr_err("not support partition =%d \n",msdc_ctl->partition);
		partitionsize = 0;
		break;
	}
#if DEBUG_MMC_IOCTL
...
static int simple_sd_ioctl_set_bootpart(struct msdc_ioctl* msdc_ctl)
{
	...
	switch (bootpart) {
	case EMMC_BOOT1_EN:
		if (0x1 != ((l_buf[EXT_CSD_PART_CFG] & 0x38) >> 3)) {
			/* active boot partition 1 */
			l_buf[EXT_CSD_PART_CFG] &= ~0x38;
			l_buf[EXT_CSD_PART_CFG] |= (0x1 << 3);
		...
		break;
	case EMMC_BOOT2_EN:
		if (0x2 != ((l_buf[EXT_CSD_PART_CFG] & 0x38) >> 3)) {
			/* actvie boot partition 2 */
			l_buf[EXT_CSD_PART_CFG] &= ~0x38;
			l_buf[EXT_CSD_PART_CFG] |= (0x2 << 3);
			pr_debug("mmc_switch set %x \n",l_buf[EXT_CSD_PART_CFG]);
			ret = mmc_switch(host_ctl->mmc->card, 0, EXT_CSD_PART_CFG, l_buf[EXT_CSD_PART_CFG], 1000);
			if (ret)
				pr_err("mmc_switch error, set boot partition \n");
		...
		break;
	case EMMC_BOOT_USER:
		if (0x7 != ((l_buf[EXT_CSD_PART_CFG] & 0x38) >> 3)) {
			/* actvie user area partition  */
			l_buf[EXT_CSD_PART_CFG] &= ~0x38;
			l_buf[EXT_CSD_PART_CFG] |= (0x7 << 3);
			pr_debug("mmc_switch set %x \n",l_buf[EXT_CSD_PART_CFG]);
			ret = mmc_switch(host_ctl->mmc->card, 0, EXT_CSD_PART_CFG, l_buf[EXT_CSD_PART_CFG], 1000);
			if (ret)
				pr_err("mmc_switch error, set boot partition \n");
